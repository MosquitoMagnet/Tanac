...
namespace Mv.Modules.P99.Hive.Services
{
    ...
    public class HiveUpload : IHiveUpload
    {
        ...
        public HiveUpload(IDataServer server, ILoggerFacade logger, IEventAggregator @event, IConfigureFile configure)
        {
            ...
            _config = configure.GetValue<P99HiveConfig>(nameof(P99HiveConfig)) ?? new P99HiveConfig();
                    ...
                    try
                    {
                       ...
                       .Where(s => s.EndsWith(".json")).ToArray();
                        ...
                        if (files.Length != 0)
                        {
                            ...
                            if (hsm.occurrence_time != null && hsm.resolved_time != null)
                            {
                ...
                else
                {
                    try
                    {
                        ...
                        if (files.Length != 0)
                        {
                            ...
                            if (hsm.input_time != null && hsm.output_time != null)
                            {
                                double total_seconds = 259200;
                                double interval_seconds = 259200;
                                try
        ...
        {
            if (e.KeyName != nameof(P99HiveConfig)) return;
            var config = _configure.GetValue<P99HiveConfig>(nameof(P99HiveConfig));
            _config = config;
        }
    }
