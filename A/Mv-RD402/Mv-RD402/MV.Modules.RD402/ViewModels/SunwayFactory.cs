...
namespace Mv.Modules.RD402.ViewModels
{
    public class SunwayFactory : IFactoryInfo
    {
        ...
        {
            string ShiftCode = "", CoilVendorCode = "";
            if (MatrixCode != null && MatrixCode.Length >=21)
            {
                ShiftCode= MatrixCode.Substring(19, 1);
                CoilVendorCode = MatrixCode.Substring(18, 1);
     
            }
            return $"{_config?.CoilWinding}{_config?.MachineCode}{ShiftCode}{CoilVendorCode}";
        }
    }
}