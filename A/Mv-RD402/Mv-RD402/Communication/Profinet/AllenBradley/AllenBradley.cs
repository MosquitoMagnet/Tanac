	...
	public class AllenBradleyNet : NetworkDeviceBase // <AllenBradleyMessage, RegularByteTransform>
	{
		...
		public virtual OperateResult<byte[]> BuildReadCommand(string[] address, int[] length)
		{
			...
			try
			{
				...
				return OperateResult.CreateSuccessResult(AllenBradleyHelper.PackRequestHeader(CipCommand, SessionHandle, commandSpecificData));
			}
			catch (Exception ex)
			{
		...
		public OperateResult<byte[]> BuildWriteCommand(string address, ushort typeCode, byte[] data, int length = 1)
		{
			try
			{
				...
				byte[] commandSpecificData = AllenBradleyHelper.PackCommandSpecificData(new byte[4], PackCommandService(PortSlot ?? new byte[] { 0x01, Slot }, cip));
		...
		{
			byte[] commandSpecificData = AllenBradleyHelper.PackCommandSpecificData(new byte[4], PackCommandService(PortSlot ?? new byte[] { 0x01, Slot }, cips.ToArray()));
			byte[] command = AllenBradleyHelper.PackRequestHeader(CipCommand, SessionHandle, commandSpecificData);
