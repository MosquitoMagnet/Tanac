...
namespace Communication.ModBus
{
	...
	public class ModbusAscii : ModbusRtu
	{
		...
		{
			// 转ascii
			byte[] modbus_ascii = ModbusInfo.TransModbusCoreToAsciiPackCommand(send);
			...
			// 还原modbus报文
			OperateResult<byte[]> modbus_core = ModbusInfo.TransAsciiPackCommandToCore(result.Content);
