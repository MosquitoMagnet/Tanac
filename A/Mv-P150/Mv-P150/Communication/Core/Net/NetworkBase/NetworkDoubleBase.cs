...
#pragma warning restore CS1998 // 异步方法缺少 "await" 运算符，将以同步方式运行
		...
		{
			OperateResult<Socket> result = await CreateSocketAndConnectAsync(new IPEndPoint(IPAddress.Parse(ipAddress), port), connectTimeOut);
			if (result.IsSuccess)
			{
				// 初始化
		...
		/// <inheritdoc cref="ReadFromCoreServer(Socket, byte[], bool)"/>
		public virtual async Task<OperateResult<byte[]>> ReadFromCoreServerAsync(Socket socket, byte[] send, bool hasResponseData = true)
		{
		...
		/// <inheritdoc cref="ReadFromCoreServer(byte[], bool)"/>
		public async Task<OperateResult<byte[]>> ReadFromCoreServerAsync(byte[] send) => await ReadFromCoreServerAsync(send, true);

		/// <inheritdoc cref="ReadFromCoreServer(byte[], bool)"/>
		public async Task<OperateResult<byte[]>> ReadFromCoreServerAsync(byte[] send, bool hasResponseData)
		{
			...
			try
			{
				...
				OperateResult<byte[]> read = await ReadFromCoreServerAsync(resultSocket.Content, send, hasResponseData);
		...
		public virtual OperateResult<byte[]> ReadFromCoreServer(Socket socket, byte[] send, bool hasResponseData = true)
		{
			LogNet?.WriteDebug(ToString(), StringResources.Language.Send + " : " + (LogMsgFormatBinary ? send.ToHexString(' ') : Encoding.ASCII.GetString(send)));
		...
		/// <inheritdoc cref="ReadFromCoreServer(byte[], bool)"/>
		public OperateResult<byte[]> ReadFromCoreServer(byte[] send) => ReadFromCoreServer(send, true);
		...
		public OperateResult<byte[]> ReadFromCoreServer(byte[] send, bool hasResponseData)
		{
			...
			try
			{
				...
				OperateResult<byte[]> read = ReadFromCoreServer(resultSocket.Content, send, hasResponseData);
