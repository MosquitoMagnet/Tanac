...
namespace Mv.Modules.P150.Services
{
    ...
    public class Upload : IUpload
    {
        ...
        public Upload(IDataServer server, ILoggerFacade logger, IEventAggregator @event, IConfigureFile configure)
        {
            this.logger = logger;
            this.server = server;
            this.@event = @event;
            ...
            while (true)
            {
                ...
                else
                {
                    try
                    {
                        ...
                        {
                            var json = JsonHelper.GetJsonFile(files[0]);
                            MachineState hsm = json.GetMachineStateModel();//将缓存区的json字符串转换为Model，然后上传
                            ...
                            }  //修改前 if(hsm.state_change_time!=null)
                            else
                            {
                                File.Delete(files[0]);
                                logger.Log("day>2" + json.Trim('\r', '\n'), Category.Debug, Priority.None);
