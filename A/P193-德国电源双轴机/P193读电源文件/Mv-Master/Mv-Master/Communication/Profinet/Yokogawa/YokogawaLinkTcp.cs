...
namespace Communication.Profinet.Yokogawa
{
	/// <summary>
	/// 横河PLC的二进制通信类，支持X,Y,I,E,M,T,C,L继电器类型的数据读写，支持D,B,F,R,V,Z,W,TN,CN寄存器类型的数据读写
	/// </summary>
	/// <remarks>
	...
	public class YokogawaLinkTcp : NetworkDeviceBase
	{
		...
		{
			OperateResult<byte[]> command = BuildWriteWordCommand(CpuNumber, address, value);
			if (!command.IsSuccess) return command;
		...
		public async override Task<OperateResult<bool[]>> ReadBoolAsync(string address, ushort length)
		{
			...
			OperateResult<byte[]> read = await ReadFromCoreServerAsync(command.Content[0]);
			if (!read.IsSuccess) return OperateResult.CreateFailedResult<bool[]>(read);

			OperateResult<byte[]> check = CheckContent(read.Content);
			if (!check.IsSuccess) return OperateResult.CreateFailedResult<bool[]>(check);

			return OperateResult.CreateSuccessResult(check.Content.Select(m => m != 0x00).ToArray());
		}
