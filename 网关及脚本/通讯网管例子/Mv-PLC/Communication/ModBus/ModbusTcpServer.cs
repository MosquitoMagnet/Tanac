...
namespace Communication.ModBus
{
	...
	/// <summary>
	///  Modbus的虚拟服务器，同时支持Tcp和Rtu的机制，支持线圈，离散输入，寄存器和输入寄存器的读写操作，同时支持掩码写入功能，可以用来当做系统的数据交换池<br />
	...
	public class ModbusTcpServer : NetworkDataServerBase
	{
		...
		private async void SocketAsyncCallBack(IAsyncResult ar)
		{
			...
			else
			{
				...
				byte[] receive = read2.Content;
				base.LogNet?.WriteDebug(ToString(), $"[{session.IpEndPoint}] Rtu {StringResources.Language.Receive}：{receive.ToHexString(' ')}");
				if (SoftCRC16.CheckCRC16(receive))
				{
					...
					{
						base.LogNet?.WriteError(ToString(), $"[{session.IpEndPoint}] Station not match Modbus-rtu : {receive.ToHexString(' ')}");
						RemoveClient(session);
						return;
					}
				...
				{
					base.LogNet?.WriteWarn($"[{session.IpEndPoint}] CRC Check Failed : {receive.ToHexString(' ')}");
				}
			}
		...
		{
			StartModbusRtu(delegate (SerialPortStream sp)
			{
				sp.PortName = com;
		...
		private void SerialPort_DataReceived(object sender, SerialDataReceivedEventArgs e)
		{
			...
			{
				base.LogNet?.WriteError(ToString(), "[" + serialPort.PortName + "] Station not match Modbus-Ascii : " + Encoding.ASCII.GetString(array2.RemoveLast(2)));
				return;
			}
			...
			base.LogNet?.WriteDebug(ToString(), "[" + serialPort.PortName + "] Ascii " + StringResources.Language.Send + "：" + Encoding.ASCII.GetString(array5.RemoveLast(2)));
			if (base.IsStarted)
			{
				RaiseDataReceived(sender, array2);
			}
