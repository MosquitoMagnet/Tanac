...
namespace DataService
{
    public abstract class ITag : IComparable<ITag>
    {
        ...
        protected Storage _oldvalue;
        public Storage OldValue
        {
            get
            {
                return _oldvalue;
        ...
        protected DateTime _oldtimestamp = DateTime.MinValue;
        ...
        public void Update(Storage newvalue, DateTime timeStamp, QUALITIES quality)
        {
            ...
            if (quality == QUALITIES.QUALITY_GOOD)
            {
                _oldvalue = _value;
                _value = newvalue;
                NotityValueChanged(new ValueChangedEventArgs(_value,_oldvalue));
        ...
        public override bool Refresh(DataSource source = DataSource.Cache)
        {
            ...
            if (_newvalue.Value != _value.Boolean)
            {
                ...
                if (_quality == QUALITIES.QUALITY_GOOD)
                {
                    _oldvalue = _value;
                    _value = value;
                    this.NotityValueChanged(new ValueChangedEventArgs(value,_oldvalue));
                }
                return true;
            }
            return false;
    ...
    public sealed class ByteTag : ITag
    {
        ...
        public override bool Refresh(DataSource source = DataSource.Device)
        {
            ...
            if (_newvalue.Value != _value.Byte)
            {
                ...
                if (_quality == QUALITIES.QUALITY_GOOD)
                {
                    _oldvalue = _value;
                    _value = value;
                    NotityValueChanged(new ValueChangedEventArgs(value,_oldvalue));
        ...
        public override int Write(object value)
        {
            ...
            _oldtimestamp = _timeStamp;
            _timeStamp = DateTime.Now;
            return _group.WriteByte(_plcAddress, temp);
        }

        protected override int InnerWrite(Storage value)
        {
            return _group.WriteByte(_plcAddress, value.Byte);
        }
    ...
    public sealed class FloatTag : ITag
    {
        ...
        {
            Storage value = Storage.Empty;
            var bytes = BitConverter.GetBytes(_group.ReadInt32(_plcAddress, source).Value);
            value.Single = BitConverter.ToSingle(bytes);//????转换错误
            return value;
        }
        ...
        public override bool Refresh(DataSource source = DataSource.Device)
        {
            ...
            if (_newvalue.Value != _str)
            {
                ...
                if (_quality == QUALITIES.QUALITY_GOOD)
                {
                    ...
                    NotityValueChanged(new ValueChangedEventArgs(_value,_value));
                }
                return true;
            }
            return false;
    ...
    public enum SeverityType : byte//新增报警程度
    {
        warning = 0,
        error = 1,
        critical = 2
    ...
    {
        public ValueChangedEventArgs(Storage value, Storage oldvalue)
        {
            this.Value = value;
            this.OldValue = oldvalue;
        }
