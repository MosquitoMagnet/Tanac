...
namespace DataService
{
    public static class ExtMethods
    {
        ...
        public static bool ModifyItemName(this ITag tag, string name)
        {
            ...
            lock (server.SyncRoot)
            {
                ...
                server.MetaDataList[index] = new TagMetaData(meta.ID, meta.GroupID, name, meta.Address, meta.Severity, meta.Code, meta.DataType, meta.Size, meta.Archive, meta.Maximum, meta.Minimum, meta.Cycle);
        ...
        public static int WriteValueEx(this IReaderWriter writer, DeviceAddress address, object value)
        {
            switch (address.VarType)
            {
                ...
                case DataType.BYTE:
                    return writer.WriteByte(address, Convert.ToByte(value));
                case DataType.WORD:
                case DataType.SHORT:
    ...
    public static class Utility
    {
        ...
        public static string ConvertToString(byte[] bytes, int start = 0, int len = 0)
        {
            ...
            return Encoding.ASCII.GetString(bytes, start, len).Trim((char)0);
        }
    }
}
