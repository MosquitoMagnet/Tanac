...
namespace Mv.Modules.TagManager.ViewModels
{
    public class TagItem : BindableBase
    {
        ...
        public TagItem(ITag tag, string tagName, string tagAddr)
        {
            ...
            this.CurrentValue = Tag.ToString();
        }
        ...
        private void TagValueChanged(object sender, ValueChangedEventArgs args)
        {
            ...
            if (Tag.GetMetaData().DataType == DataType.INT)
            {
                short ex = (short)Tag.GetMetaData().Zoom;
                CurrentValue = ((double)Tag.Value.Int32/ex).ToString();
            }
               
            else if(Tag.GetMetaData().DataType==DataType.SHORT)
            {
                short ex = (short)Tag.GetMetaData().Zoom;
                CurrentValue = ((double)Tag.Value.Int16 / ex).ToString();
            }
            else
            {
                CurrentValue = Tag.ToString();
            }
