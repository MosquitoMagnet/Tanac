...
{
    [Description("Delta-Dvp-Rtu协议 未验证")]
    public class DeltaDvpRtuDriver : DriverInitBase, IPLCDriver, IMultiReadWrite
    {

        protected Dictionary<int, DeltaDataType> _dictionary = new Dictionary<int, DeltaDataType>()
        {
        };
        protected DeltaDvpSerial mc = new DeltaDvpSerial();
        public void Dispose()
        {
            mc.Close();
            //  throw new System.NotImplementedException();
        }
        public DeltaDvpRtuDriver()
        {

        }
        public DeltaDvpRtuDriver(IDataServer server, short id, string name, string serverName, int timeOut = 500, IDictionary<string, string> paras = null) : base(server, id, name, serverName, timeOut, paras)
        {
            ID = id;
            Name = name;
            Parent = server;
