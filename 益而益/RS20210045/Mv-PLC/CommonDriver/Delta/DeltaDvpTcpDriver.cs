...
{
    [Description("Delta-Dvp-Tcp协议 未验证")]
    public class DeltaDvpTcpDriver : DriverInitBase, IPLCDriver, IMultiReadWrite
    {

        protected Dictionary<int, DeltaDataType> _dictionary = new Dictionary<int, DeltaDataType>()
        {
        };
        protected DeltaDvpTcpNet mc = new DeltaDvpTcpNet();
        public void Dispose()
        {
            mc.ConnectClose();
            //  throw new System.NotImplementedException();
        }
        public DeltaDvpTcpDriver()
        {

        }
        public DeltaDvpTcpDriver(IDataServer server, short id, string name, string serverName, int timeOut = 500, IDictionary<string, string> paras = null) : base(server, id, name, serverName, timeOut, paras)
        {
            ID = id;
            Name = name;
            Parent = server;
