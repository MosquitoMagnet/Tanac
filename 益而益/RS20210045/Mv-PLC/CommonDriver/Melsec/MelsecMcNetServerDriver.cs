...
namespace CommonDriver.Melsec
{
    [Description("Melsec-MC-3E-Server")]
    public class MelsecMcNetServerDriver : DriverInitBase, IPLCDriver, IMultiReadWrite
    {

        protected Dictionary<int, MelsecMcDataType> _dictionary = new Dictionary<int, MelsecMcDataType>()
        {
        };
        protected MelsecMcServer mc = new MelsecMcServer();
        public void Dispose()
        {
            mc.ServerClose();
            //  throw new System.NotImplementedException();
        }
        public MelsecMcNetServerDriver()
        {

        }
        public MelsecMcNetServerDriver(IDataServer server, short id, string name, string serverName, int timeOut = 500, IDictionary<string, string> paras = null) : base(server, id, name, serverName, timeOut, paras)
        {
            ID = id;
            Name = name;
            Parent = server;
