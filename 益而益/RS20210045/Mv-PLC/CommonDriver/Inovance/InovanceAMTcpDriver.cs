...
{
    [Description("INOVANCE_A400_800_ModbusTCP 未验证")]
    public class InovanceAMTcpDriver : DriverInitBase, IPLCDriver, IMultiReadWrite
    {

        protected Dictionary<int, InovanceDataType> _dictionary = new Dictionary<int, InovanceDataType>()
        {
        };
        protected InovanceAMTcp mc = new InovanceAMTcp();
        public void Dispose()
        {
            mc.ConnectClose();
        ...
        public virtual DeviceAddress GetDeviceAddress(string address)
        {
            ...
            OperateResult<InovanceAddressData> operateResult = InovanceAddressData.ParseAMFrom(address, 0);
        ...
        public string GetAddress(DeviceAddress address)
        {
            ...
            if (m != null)
            {
                if (m.DataType == 0)
                    ...
                    return $"{m.AsciiCode.Trim('*')}{Convert.ToString(address.Start*8 + address.Bit, m.FromBase)}";
            }
            else
            {
                return null;
