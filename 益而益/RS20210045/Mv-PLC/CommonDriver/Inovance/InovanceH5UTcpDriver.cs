...
{
    [Description("INOVANCE_H5U_ModbusTCP 未验证")]
    public class InovanceH5UTcpDriver : DriverInitBase, IPLCDriver, IMultiReadWrite
    {

        protected Dictionary<int, InovanceDataType> _dictionary = new Dictionary<int, InovanceDataType>()
        {
        };
        protected InovanceH5UTcp mc = new InovanceH5UTcp();
        public void Dispose()
        {
            mc.ConnectClose();
            //  throw new System.NotImplementedException();
        }
        public InovanceH5UTcpDriver()
        {

        }
        public InovanceH5UTcpDriver(IDataServer server, short id, string name, string serverName, int timeOut = 500, IDictionary<string, string> paras = null) : base(server, id, name, serverName, timeOut, paras)
        {
            ID = id;
            Name = name;
            Parent = server;
        ...
        public virtual DeviceAddress GetDeviceAddress(string address)
        {
            ...
            OperateResult<InovanceAddressData> operateResult = InovanceAddressData.ParseH5UFrom(address, 0);
