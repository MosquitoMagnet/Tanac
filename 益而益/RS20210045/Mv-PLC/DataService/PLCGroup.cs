...
namespace DataService
{
    public class PLCGroup : IGroup
    {
        ...
        protected virtual void UpdatePDUArea()
        {
            ...
            foreach (PDUArea area in _rangeList)
            {
                ...
                else
                {
                    ...
                    while (index < count)
                    {
                        ...
                        int iByte = addr.CacheIndex; // 缓存索引 将数据存入缓存中，根据缓存索引来索引缓存中数据位置
        ...
        public int WriteByte(DeviceAddress address, byte value)
        {
            int rs = _plcReader.WriteByte(address, value);
            if (rs >= 0)
            {
                if (DataChange != null)
                {
