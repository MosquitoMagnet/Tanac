...
namespace Communication.Core
{
	...
	public static class NetSupport
	{
		...
		/// </summary>
		internal const int SocketBufferSize = 8192;
		...
		internal static byte[] ReadBytesFromSocket(Socket socket, int receive, Action<long, long> reportProgress = null)
		{
			...
			while (count_receive < receive)
			{
				...
				int count = socket.Receive(bytes_receive, count_receive, receive_length, SocketFlags.None);
				count_receive += count;

				if (count == 0) throw new RemoteCloseException();

				reportProgress?.Invoke(count_receive, receive);
			}
