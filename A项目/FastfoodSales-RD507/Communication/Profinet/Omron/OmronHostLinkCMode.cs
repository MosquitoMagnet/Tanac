...
namespace Communication.Profinet.Omron
{
	...
	public class OmronHostLinkCMode : SerialDeviceBase
	{
		...
		public override OperateResult<byte[]> Read(string address, ushort length)
		{
			...
			// 核心交互
			OperateResult<byte[]> read = ReadBase(PackCommand(command.Content, UnitNumber));
		...
		{
			// 核心数据交互
			OperateResult<byte[]> read = ReadBase(PackCommand(Encoding.ASCII.GetBytes("MM"), UnitNumber));
